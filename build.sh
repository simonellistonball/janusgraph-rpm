...
OWNER_UID=`ls -n SPECS/janusgraph.spec | awk -F' ' '{ print $3 }'`
...
QA_SKIP_BUILD_ROOT=1 rpmbuild -v -ba --define "_topdir $(pwd)" --define "_version ${VERSION}" --define "_prerelease ${PRERELEASE}" SPECS/janusgraph.spec
if [ $? -ne 0 ]; then
  echo "RPM build errors encountered" >&2
  exit 1
fi

rpmlint -i SPECS/janusgraph.spec RPMS/*/janusgraph* SRPMS/janusgraph
